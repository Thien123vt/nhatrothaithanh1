  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        ...
        <div className="bg-white px-4 py-3 rounded-2xl border border-slate-100 flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-3xl border border-slate-200">
          ...
          <div className="space-y-5">
            {[...currentData].sort((a,b) => (b.newElectricity-b.oldElectricity) - (a.newElectricity-a.oldElectricity)).map(d => {
              const used = Math.max(0, d.newElectricity - d.oldElectricity);
              const percentage = (used / maxElec) * 100;
              return (
                <div key={d.roomId}>
                  <div className="flex justify-between text-[0.8em] font-bold text-slate-500 mb-1">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200">
          ...
          <div className="space-y-5">
            {[...currentData].sort((a,b) => (b.newWater-b.oldWater) - (a.newWater-a.oldWater)).map(d => {
...
const StatCard: React.FC<{ title: string, value: string, icon: React.ReactNode, color: string }> = ({ title, value, icon, color }) => (
  <div className={`${color} border p-6 rounded-3xl shadow-sm transition-transform hover:scale-[1.02]`}>
    <div className="p-3 bg-white/60 rounded-2xl mb-4 w-fit">{icon}</div>
    <div className="text-[0.6em] font-black text-slate-500 uppercase tracking-widest mb-1">{title}</div>
    <div className="text-[1.5em] font-black text-slate-800">{value}</div>
  </div>
);
